#ifndef SPECTRA_SYM_EIGS_BASE_CUH
#define SPECTRA_SYM_EIGS_BASE_CUH
...
private:
    ...
    using LanczosFac = Spectra::Lanczos<Scalar, ArnoldiOpType>;
...
private:
    ...
    Spectra::CompInfo      m_info;       // status of the computation
    // clang-format on
    ...
    void restart(Index k, Spectra::SortRule selection)
    {
        using std::abs;
        ...
        Spectra::TridiagQR<Scalar> decomp(m_ncv);
    ...
    Index num_converged(const Scalar& tol)
    {
        ...
        constexpr Scalar eps = Spectra::TypeTraits<Scalar>::epsilon();
    ...
    Index nev_adjusted(Index nconv)
    {
        ...
        constexpr Scalar near_0 = Spectra::TypeTraits<Scalar>::min() * Scalar(10);
    ...
    void retrieve_ritzpair(Spectra::SortRule selection)
    {
        Spectra::TridiagEigen<Scalar> decomp(m_fac.matrix_H());
...
protected:
    ...
    virtual void sort_ritzpair(Spectra::SortRule sort_rule)
    {
        if ((sort_rule != Spectra::SortRule::LargestAlge) && (sort_rule != Spectra::SortRule::LargestMagn) &&
            (sort_rule != Spectra::SortRule::SmallestAlge) && (sort_rule != Spectra::SortRule::SmallestMagn))
...
public:
    ...
    SymEigsBase(OpType& op, const BOpType& Bop, Index nev, Index ncv) :
        ...
        m_info(Spectra::CompInfo::NotComputed)
    {
        if (nev < 1 || nev > m_n - 1)
    ...
    void init()
    {
        Spectra::SimpleRandom<Scalar> rng(0);
    ...
    ///
    Index compute(Spectra::SortRule selection = Spectra::SortRule::LargestMagn, Index maxit = 1000,
                  Scalar tol = 1e-10, Spectra::SortRule sorting = Spectra::SortRule::LargestAlge)
    {
        ...
        for (i = 0; i < maxit; i++)
        {
            ...
            std::cout << "\r#    iter ... " << std::setw(4) << i 
		<< " , nconv : " << std::setw(4) << nconv << std::flush;
            if (nconv >= m_nev)
                break;
        ...
        m_niter += i + 1;
        m_info = (nconv >= m_nev) ? Spectra::CompInfo::Successful : Spectra::CompInfo::NotConverging;
...
#endif  // SPECTRA_SYM_EIGS_BASE_CUH
